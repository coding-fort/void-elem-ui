...
<script>
  ...
  import VoImageFormItem from "../image-form-item/index.vue";
  export default {
    name: "VoFormItem",
    components: {
      VoInputRange,
    ...
    methods: {
       ...
       */
      handleChange(value) {
        // console.log("chang:formitem", this.type);
        let _value = this.form[this.field];
        // 文件上传
        if (this.type == "upload") {
          // console.log("change:formitem:upload", value);
          _value = value;
          this.form[this.field] = value;
          this.$nextTick(() => {
            this.validate(); // 移除/触发校验
          });
        }
        this.$emit("change", {
          value: _value,
        ...
        // if (
        //   !value &&
        //   ["monthrange", "daterange", "datetimerange"].includes(this.type)
        // ) {
      ...
      handleClear() {
        // this.$emit("clear");
        this.$emit("change", { value: null, key: this.field });
      },
      /**
       * 日期时间控件格式化预处理
       */
