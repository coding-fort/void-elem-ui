<template>
  <comp-wrap title="RSA 解密（RSATool.decrypt）" :desc="desc" :code-str="codeStr">
    <p class="mt-0 mb-10">
      1.RSA 私钥
...
this.encrypted = await RSATool.decrypt(this.encrypted, this.privateKey);
          `,
    ...
    methods: {
      async handleDecrypt() {
        ...
        try {
          this.decrypted = await RSATool.decrypt(
            this.encrypted,
            this.privateKey
          );
