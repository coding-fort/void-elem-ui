  ...
  >
    ...
    <ul class="mt-10">
      <li>
        <code
          >const {start, end} =
          this.$vutil.date.transformQueryDate(dateArr)</code
        >
      </li>
      <li>
...
methods: {
  ...
  handleTransfer() {
    ...
    this.transferDaterange = this.$vutil.date.transformQueryDate(
      this.daterange,
      this.options,
    );
  ...
  handleSetOption(type) {
    this.value = "";
    const { formatType, formatStr, autoAddTime } = this.options;
